...
import { ArrowLeft, Search, Shield } from 'lucide-react-native';
...
export default function DisplayBuildingSecurity() {
    ...
    try {
      ...
      try {
        ...
        if (userDataString) {
          ...
          parsedData.isTemporary = false;
          await SecureStore.setItemAsync('userData', JSON.stringify(parsedData));
        } else {
          // Create new userData if it doesn't exist
          await SecureStore.setItemAsync('userData', JSON.stringify({ buildingId: building._id, isTemporary: false }));
        }
      } catch (storageError) {
        console.error('Error updating SecureStore:', storageError);
      }

      // Navigate to security page after successful building selection
      router.push('./securite');
    } catch (error) {
      console.error('Error selecting building:', error);
      const errorMessage = error instanceof Error ? error.message : 'Erreur inconnue';
      showAlert('Erreur', `Impossible de sélectionner l'immeuble: ${errorMessage}`, 'error');
    }
  };
  ...
  const renderItem = ({ item }: { item: BuildingType }) => (
    ...
    >
      <Shield size={24} color="#22C55E" style={styles.buildingIcon} />
